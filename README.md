...
- 2025/10/31
  ...
  ```
  1、新增目录扫描模块：-m port -u 10.0.0.0/24 -p 80,443
...
- 2025/10/29
  ...
  3、新增--rate参数，-p参数，用来指定端口扫描的速率以及扫描端口的范围。命令行包含-p参数才会运行指纹识别模块。
  ```
...
| 参数 | 默认值 | 说明 | 示例 |
|------|--------|------|------|
| `-u` | 必填或 `-f` | 目标列表，逗号分隔。支持完整 URL、域名、`host:port`、CIDR、IP 范围。 | `-u http://a.com,https://b.com` |
| `-f` | — | 目标文件路径，每行一个目标；支持注释 `#` 和空行。 | `-f targets.txt` |
